...
class Connection(sqlite3.Connection):
    def __init__(self, db_file, **kwargs):
        ...
        if not db_exists:
            os.fdopen(os.open(db_file, os.O_WRONLY | os.O_CREAT, 0o600),
                      'w').close()
...
class SQLiteConnection(object):
    ...
    def __init__(self, db_file):
        ...
        if os.path.splitext(db_file)[1] not in ['.nixops', '.charon']:
            raise Exception(
                "state file ‘{0}’ should have extension ‘.nixops’".format(
                    db_file))
        db = sqlite3.connect(
            self.db_file,
            timeout=60,
        ...
        with db:
            ...
            if version == self.current_schema:
                ...
                c.execute("update SchemaVersion set version = ?",
                          (self.current_schema, ))
            else:
                raise Exception(
                    "this NixOps version is too old to deal with schema version {0}".
                    format(version))
    ...
    def _find_deployment(self, uuid=None):
        ...
        if len(res) == 0:
            if uuid:
                ...
                c.execute("select uuid from Deployments where uuid glob ?",
                          (uuid + '*', ))
                res = c.fetchall()
                if len(res) == 0:
                    return None
    ...
    def create_deployment(self, uuid=None):
        ...
        if not uuid:
            ...
            uuid = str(uuid.uuid1())
        with self.db:
            self.db.execute("insert into Deployments(uuid) values (?)",
                            (uuid, ))
    ...
    def _delete_deployment(self, deployment_uuid):
        ...
        with self.db:
            self.db.execute("delete from Deployments where uuid = ?",
                            (deployment_uuid, ))
    ...
    def _create_schemaversion(self, c):
        c.execute('''create table if not exists SchemaVersion(
                 version integer not null
               );''')

        c.execute("insert into SchemaVersion(version) values (?)",
                  (self.current_schema, ))
    ...
    def _create_schema(self, c):
        ...
        c.execute('''create table if not exists ResourceAttrs(
            ...
            for name, value in attrs.iteritems():
                if value == None:
                    self.db.execute(
                        ...
                        (deployment_uuid, name))
                else:
                    if self.get_deployment_attr(deployment_uuid,
                                                name) == nixops.util.undefined:
                        self.db.execute(
                            "insert into DeploymentAttrs(deployment, name, value) values (?, ?, ?)",
                            (deployment_uuid, name, value))
                    else:
                        self.db.execute(
                            "update DeploymentAttrs set value=? where deployment=? and name=?",
                            (value, deployment_uuid, name))

    def del_deployment_attr(self, deployment_uuid, attr_name):
        with self.db:
            self.db.execute(
                "delete from DeploymentAttrs where deployment = ? and name = ?",
                (deployment_uuid, attr_name))
...
#     ## Resources
    ...
    def create_resource(self, deployment, name, type):
        count = self.db.execute(
            "select count(id) from Resources where deployment = ? and name = ?",
            (deployment.uuid, name)).fetchone()[0]
    ...
    def _rename_resource(self, deployment_uuid, resource_id, new_name):
        ...
        with self.db:
            self.db.execute(
                ...
                (new_name, deployment_uuid, resource_id))
        ...
        with self.db:
            for name, value in attrs.iteritems():
                if value == None:
                    ...
                    if self.get_resource_attr(_deployment_uuid, resource_id,
                                              name) == nixops.util.undefined:
                        self.db.execute(
                            "insert into ResourceAttrs(machine, name, value) values (?, ?, ?)",
                            (resource_id, name, value))
                    else:
                        self.db.execute(
                            "update ResourceAttrs set value=? where machine=? and name=?",
    ...
    def get_all_resource_attrs(self, deployment_uuid, resource_id):
        with self.db:
            rows = self.db.execute(
                "select name, value from ResourceAttrs where machine = ?",
