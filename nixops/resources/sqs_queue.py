...
class SQSQueueDefinition(nixops.resources.ResourceDefinition):
    ...
    def __init__(self, xml):
        ...
        self.visibility_timeout = xml.find(
            "attrs/attr[@name='visibilityTimeout']/int").get("value")
...
class SQSQueueState(nixops.resources.ResourceState):
    ...
    state = nixops.util.attr_property("state",
                                      nixops.resources.ResourceState.MISSING,
                                      int)
    queue_name = nixops.util.attr_property("ec2.queueName", None)
    access_key_id = nixops.util.attr_property("ec2.accessKeyId", None)
    region = nixops.util.attr_property("ec2.region", None)
    visibility_timeout = nixops.util.attr_property(
        "ec2.queueVisibilityTimeout", None)
    ...
    def create(self, defn, check, allow_reboot, allow_recreate):
        ...
        if self.state == self.UP and (self.queue_name != defn.queue_name or
        ...
        if check or self.state != self.UP:
            ...
            if not q or self.state != self.UP:
                if q:
                    ...
                    # recreate a queue.
                    self.log("deleting queue ‘{0}’ (and waiting 60 seconds)"
                             "...".format(defn.queue_name))
                    self._conn.delete_queue(q)
                    time.sleep(61)
                self.log("creating SQS queue ‘{0}’...".format(defn.queue_name))
                q = nixops.ec2_utils.retry(
