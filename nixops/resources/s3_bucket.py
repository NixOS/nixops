...
class S3BucketState(nixops.resources.ResourceState):
    ...
    def create(self, defn, check, allow_reboot, allow_recreate):
        ...
        if check or self.state != self.UP:
            ...
            try:
                self._conn.create_bucket(
                    defn.bucket_name,
                    location=region_to_s3_location(defn.region))
    ...
    def destroy(self):
        if self.state == self.UP:
            self.connect()
            try:
                self.log("destroying S3 bucket ‘{0}’"
                         "...".format(self.bucket_name))
                bucket = self._conn.get_bucket(self.bucket_name)
                try:
                    bucket.delete()
                except boto.exception.S3ResponseError as e:
                    if e.error_code != "BucketNotEmpty":
                        raise
                    if not self.depl.confirm(
                            "are you sure you want to destroy S3 bucket ‘{0}’"
                            "?".format(self.bucket_name)):
                        return False
                    keys = bucket.list()
