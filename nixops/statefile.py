    ...
    if not os.path.exists(home):
        ...
        if os.path.exists(old_home):
            ...
            if os.path.exists(home + "/deployments.charon"):
                os.rename(home + "/deployments.charon",
                          home + "/deployments.nixops")
        else:
            os.makedirs(home, 0700)
    return os.environ.get("NIXOPS_STATE",
                          os.environ.get("CHARON_STATE",
                                         home + "/deployments.nixops"))
...
class StateFile(object):
    ...
    def __init__(self, db_file):
        ...
        if os.path.splitext(db_file)[1] not in ['.nixops', '.charon']:
            raise Exception("state file ‘{0}’ should have extension "
                            "‘.nixops’".format(db_file))
        db = sqlite3.connect(db_file, timeout=60, check_same_thread=False,
                             factory=Connection)  # FIXME
        db.db_file = db_file
        ...
        with db:
            ...
            if version == self.current_schema:
                ...
                raise Exception("this NixOps version is too old to deal with "
                                "schema version {0}".format(version))
    ...
    def get_all_deployments(self):
        ...
        for uuid in uuids:
            try:
                ...
                sys.stderr.write("skipping deployment ‘{0}’: "
                                 "{1}\n".format(uuid, str(e)))
        return res
    ...
    def _find_deployment(self, uuid=None):
        ...
        if not uuid:
            ...
            c.execute("select uuid from Deployments d where uuid = ? or "
                      "exists (select 1 from DeploymentAttrs where "
                      "deployment = d.uuid and name = 'name' and "
                      "value = ?)", (uuid, uuid))
        res = c.fetchall()
        if len(res) == 0:
            return None
        if len(res) > 1:
            if uuid:
                raise Exception("state file contains multiple deployments "
                                "with the same name, so you should specify "
                                "one using its UUID")
            else:
                raise Exception("state file contains multiple deployments, "
                                "so you should specify which one to use "
                                "using ‘-d’, or set the environment "
                                "variable NIXOPS_DEPLOYMENT")
    ...
    def open_deployment(self, uuid=None):
        ...
        if deployment:
            return deployment
        raise Exception("could not find specified deployment in state file "
                        "‘{0}’".format(self.db_file))
    ...
    def create_deployment(self, uuid=None):
        ...
        if not uuid:
            ...
            uuid = str(uuid.uuid1())
        with self._db:
            self._db.execute("insert into Deployments(uuid) values "
                             "(?)", (uuid,))
    ...
    def _create_schema(self, c):
        ...
        c.execute(
            '''create table if not exists DeploymentAttrs(
                 ...
                 foreign key(deployment) references Deployments(uuid)
                    on delete cascade
               );''')
        ...
        c.execute(
            '''create table if not exists ResourceAttrs(
                 ...
                 foreign key(machine) references Resources(id)
                    on delete cascade
               );''')
