...
class LibvirtdState(MachineState):
        ...
        if self.vm_id is None:
            ...
            base_image = self._logged_exec(
                        ...
                        ["--arg", "checkConfigurationOptions", "false",
                         "-A", "nodes.{0}.config.deployment.libvirtd.baseImage"
                                    .format(self.name),
                         "-o", "{0}/libvirtd-image-{1}"
                                    .format(self.depl.tempdir, self.name)],
    ...
    def _make_domain_xml(self, defn):
        ...
        return domain_fmt.format(self._vm_id(), defn.memory_size
                    , qemu_kvm, self._disk_path(defn))
