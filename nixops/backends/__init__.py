...
class MachineDefinition(nixops.resources.ResourceDefinition):
    ...
    def __init__(self, xml):
        ...
        self.encrypted_links_to = set([e.get("value") for e in xml.findall(
            "attrs/attr[@name='encryptedLinksTo']/list/string")])
        self.store_keys_on_machine = xml.find(
            "attrs/attr[@name='storeKeysOnMachine']/bool").get(
                "value") == "true"
        self.keys = {k.get("name"): k.find("string").get("value")
                     for k in xml.findall(
                         "attrs/attr[@name='keys']/attrs/attr")}
        self.owners = [e.get("value") for e in xml.findall(
            "attrs/attr[@name='owners']/list/string")]
    ...
    def __init__(self, tempdir, name, ssh_name, ssh_flags):
        ...
        res = subprocess.call(
            ["ssh", "-x", "root@" + self._ssh_name, "-S", self._control_socket,
             ...
             '-oServerAliveInterval=60'] + ssh_flags)
        if res != 0:
            raise SSHConnectionFailed("unable to start SSH master connection "
                                      "to ‘{0}’".format(name))
...
class MachineState(nixops.resources.ResourceState):
    ...
    store_keys_on_machine = nixops.util.attr_property(
        "storeKeysOnMachine", True, bool)
    ...
    def get_load_avg(self):
        ...
        try:
            res = self.run_command("cat /proc/loadavg", capture_stdout=True,
                                   timeout=15).rstrip().split(' ')
            assert len(res) >= 3
            return res
    ...
    def _check(self, res):
        ...
        if avg is None:
            ...
            out = self.run_command("systemctl --all --full",
            ...
            for l in out:
                ...
                if match and not match.group(1).startswith("sys-") and not\
                        match.group(1).startswith("dev-"):
                    res.failed_units.append(match.group(1))

    def restore(self, defn, backup_id, devices=[]):
        """Restore persistent disks to a given backup, if possible."""
        self.warn("don't know how to restore disks from backup for machine "
                  "‘{0}’".format(self.name))

    def remove_backup(self, backup_id):
        """Remove a given backup of persistent disks, if possible."""
        self.warn("don't know how to remove a backup for machine "
                  "‘{0}’".format(self.name))

    def backup(self, defn, backup_id):
        """Make backup of persistent disks, if possible."""
        self.warn("don't know how to make backup of disks for machine "
                  "‘{0}’".format(self.name))
    ...
    def reboot_sync(self):
        ...
        nixops.util.wait_for_tcp_port(self.get_ssh_name(), 22, open=False,
                                      callback=lambda: self.log_continue("."))
        self.log_continue("[down]")
        nixops.util.wait_for_tcp_port(self.get_ssh_name(), 22,
                                      callback=lambda: self.log_continue("."))
        self.log_end("[up]")
        self.state = self.UP
        self.ssh_pinged = True
    ...
    def address_to(self, m):
        """Return the IP address to be used to access machone "m" from this
           machine.
        """
        ip = m.public_ipv4
        if ip:
    ...
    def _open_ssh_master(self, timeout=None):
        ...
        while True:
            try:
                self.ssh_master = SSHMaster(
                    self.depl.tempdir, self.name, self.get_ssh_name(),
                    self.get_ssh_flags() + ([
                        "-o", "ConnectTimeout={0}".format(timeout)]
                        if timeout else []))
                break
            except Exception:
                tries = tries - 1
    ...
    def write_ssh_private_key(self, private_key):
        ...
        with os.fdopen(os.open(
                key_file, os.O_CREAT | os.O_WRONLY, 0600), "w") as f:
            f.write(private_key)
    ...
    def _logged_exec(self, command, check=True, capture_stdout=False,
        ...
        for fd in fds:
            nixops.util.make_non_blocking(fd)
        ...
        if stdin_string is not None:
            process.stdin.close()
        if check and res != 0:
            raise SSHCommandFailed("command ‘{0}’ failed on machine "
    ...
    def copy_closure_to(self, path):
        ...
        if not self.has_really_fast_connection():
            closure = subprocess.check_output([
                "nix-store", "-qR", path]).splitlines()
            self.run_command("nix-store -j 4 -r --ignore-unknown " +
    ...
    def generate_vpn_key(self):
        ...
        if self.public_vpn_key and _vpn_key_exists:
            return
        ...
        f.close()
        if res != 0:
            raise Exception("unable to upload VPN key to "
                            "‘{0}’".format(self.name))
    ...
    def upload_file(self, source, target, recursive=False):
        ...
        if recursive:
            ...
            recursive_cmdline = []
        cmdline = ["scp"] + self.get_ssh_flags() + recursive_cmdline + [
            source, "root@" + self.get_ssh_name() + ":" + target]
    ...
    def download_file(self, source, target, recursive=False):
        ...
        if recursive:
            ...
            recursive_cmdline = []
        cmdline = ["scp"] + self.get_ssh_flags() + recursive_cmdline + [
            "root@" + self.get_ssh_name() + ":" + source, target]
        return self._logged_exec(cmdline)
...
def create_definition(xml):
    """Create a machine definition object from the given XML representation of
       the machine's attributes.
    """
    ...
    for i in [nixops.backends.none.NoneDefinition,
        ...
        if target_env == i.get_type():
            return i(xml)
    raise nixops.deployment.UnknownBackend("unknown backend type "
                                           "‘{0}’".format(target_env))
