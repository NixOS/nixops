...
class EC2Definition(MachineDefinition):
    ...
    def __init__(self, xml):
        ...
        self.access_key_id = x.find(
            "attr[@name='accessKeyId']/string").get("value")
        ...
        self.controller = x.find(
            "attr[@name='controller']/string").get("value")
        self.ami = x.find("attr[@name='ami']/string").get("value")
        if self.ami == "":
            raise Exception("no AMI defined for EC2 machine "
                            "‘{0}’".format(self.name))
        self.instance_type = x.find(
            "attr[@name='instanceType']/string").get("value")
        self.key_pair = x.find("attr[@name='keyPair']/string").get("value")
        self.private_key = x.find(
            "attr[@name='privateKey']/string").get("value")
        self.security_groups = [e.get("value") for e in x.findall(
            "attr[@name='securityGroups']/list/string")]
        self.instance_profile = x.find(
            "attr[@name='instanceProfile']/string").get("value")
        self.tags = {k.get("name"): k.find("string").get("value")
                     for k in x.findall("attr[@name='tags']/attrs/attr")}
        self.root_disk_size = int(x.find(
            "attr[@name='ebsInitialRootDiskSize']/int").get("value"))

        def f(xml):
            return {
                'disk': xml.find(
                    "attrs/attr[@name='disk']/string").get("value"),
                'size': int(xml.find(
                    "attrs/attr[@name='size']/int").get("value")),
                'iops': int(xml.find(
                    "attrs/attr[@name='iops']/int").get("value")),
                'fsType': xml.find(
                    "attrs/attr[@name='fsType']/string").get("value"),
                'deleteOnTermination': xml.find(
                    "attrs/attr[@name='deleteOnTermination']/bool").get(
                        "value") == "true",
                'encrypt': xml.find(
                    "attrs/attr[@name='encrypt']/bool").get(
                        "value") == "true",
                'passphrase': xml.find(
                    "attrs/attr[@name='passphrase']/string").get("value")}

        self.block_device_mapping = {
            _xvd_to_sd(k.get("name")): f(k) for k in x.findall(
                "attr[@name='blockDeviceMapping']/attrs/attr")}
        self.elastic_ipv4 = x.find(
            "attr[@name='elasticIPv4']/string").get("value")

        x = xml.find("attrs/attr[@name='route53']/attrs")
        assert x is not None
        self.dns_hostname = x.find(
            "attr[@name='hostName']/string").get("value")
        self.dns_ttl = x.find("attr[@name='ttl']/int").get("value")
        self.route53_access_key_id = x.find(
            "attr[@name='accessKeyId']/string").get("value")

    def show_type(self):
        return "{0} [{1}]".format(self.get_type(),
                                  self.region or self.zone or "???")
...
class EC2State(MachineState):
    ...
    controller = nixops.util.attr_property(
        "ec2.controller", None)  # FIXME: not used
    ...
    security_groups = nixops.util.attr_property(
        "ec2.securityGroups", None, 'json')
    ...
    route53_access_key_id = nixops.util.attr_property(
        "route53.accessKeyId", None)
    ...
    def get_ssh_name(self):
        if not self.public_ipv4:
            raise Exception("EC2 machine ‘{0}’ does not have a public "
                            "IPv4 address (yet)".format(self.name))
    ...
    def get_ssh_private_key_file(self):
        ...
        for r in self.depl.active_resources.itervalues():
            if isinstance(r, nixops.resources.ec2_keypair.EC2KeyPairState) and\
                    r.state == nixops.resources.ec2_keypair.EC2KeyPairState.UP\
    ...
    def get_physical_spec(self):
        ...
        for k, v in self.block_device_mapping.items():
            if v.get('encrypt', False) and v.get('passphrase', "") == "" and\
                    v.get('generatedKey', "") != "":
                lines.append('    deployment.ec2.blockDeviceMapping."{0}".'
                             'passphrase = pkgs.lib.mkOverride 10 "{1}";'
    ...
    def get_physical_backup_spec(self, backupid):
        if backupid in self.backups:
            return ['    deployment.ec2.blockDeviceMapping."{0}".disk = '
                    'pkgs.lib.mkStrict "{1}";'.format(
                        _sd_to_xvd(dev), snap)
    ...
    def get_keys(self):
        ...
        for k, v in self.block_device_mapping.items():
            if v.get('encrypt', False) and v.get('passphrase', "") == "" and\
                    v.get('generatedKey', "") != "":
                keys["luks-" + _sd_to_xvd(k).replace('/dev/', '')] =\
                    v['generatedKey']
        return keys
    ...
    def show_type(self):
        ...
        if self.zone or self.region:
            s = "{0} [{1}; {2}]".format(
                s, self.zone or self.region, self.instance_type)
        return s
    ...
    def connect_route53(self):
        ...
        (access_key_id, secret_access_key) =\
            nixops.ec2_utils.fetch_aws_secret_key(self.route53_access_key_id)

        self._conn_route53 = boto.connect_route53(access_key_id,
                                                  secret_access_key)
    ...
    def _get_instance_by_id(self, instance_id, allow_missing=False):
        ...
        if len(reservations) == 0:
            if allow_missing:
                return None
            raise EC2InstanceDisappeared("EC2 instance ‘{0}’ disappeared"
                                         "!".format(instance_id))
    ...
    def _get_volume_by_id(self, volume_id, allow_missing=False):
        ...
        try:
            ...
            if len(volumes) != 1:
                raise Exception("unable to find volume "
                                "‘{0}’".format(volume_id))
            return volumes[0]
    ...
    def _get_snapshot_by_id(self, snapshot_id):
        ...
        if len(snapshots) != 1:
            raise Exception("unable to find snapshot "
    ...
    def _wait_for_ip(self, instance):
        ...
        while True:
            ...
            if instance.state not in {"pending", "running", "scheduling",
                                      "launching", "stopped"}:
                raise Exception("EC2 instance ‘{0}’ failed to start (state is "
                                "‘{1}’)".format(self.vm_id, instance.state))
        ...
        self.log_end("{0} / {1}".format(instance.ip_address,
    ...
    def get_backups(self):
        self.connect()
        backups = {}
        set([v['volumeId'] for v in self.block_device_mapping.values()])
        for b_id, b in self.backups.items():
            ...
            for k, v in self.block_device_mapping.items():
                if not k in b.keys():
                    ...
                    info.append("{0} - {1} - Not available in "
                                "backup".format(self.name, k))
                else:
                    snapshot_id = b[k]
                    try:
                        ...
                        if snapshot_status != '100%':
                            info.append("progress[{0},{1},{2}] = {3}%"
                                        ".".format(self.name, k, snapshot_id,
                                                   snapshot_status))
                            backup_status = "running"
                    except:
                        info.append("{0} - {1} - {2} - Snapshot has "
                                    "disappeared".format(self.name, k,
                                                         snapshot_id))
        ...
        if not backup_id in _backups.keys():
            ...
            for dev, snapshot_id in _backups[backup_id].items():
                ...
                try:
                    ...
                    self.warn('snapshot {0} not found, skipping'
                              '.'.format(snapshot_id))
    ...
    def get_common_tags(self):
        return {'CharonNetworkUUID': self.depl.uuid,
                ...
                'CharonStateFile': "{0}@{1}:{2}".format(
                    getpass.getuser(),
                    socket.gethostname(),
                    self.depl._db.db_file)}

    def backup(self, defn, backup_id):
        self.connect()

        self.log("backing up machine ‘{0}’ using id "
                 "‘{1}’".format(self.name, backup_id))
        backup = {}
        _backups = self.backups
        for k, v in self.block_device_mapping.items():
            snapshot = nixops.ec2_utils.retry(
                lambda: self._conn.create_snapshot(volume_id=v['volumeId']))
            self.log("+ created snapshot of volume ‘{0}’: "
                     "‘{1}’".format(v['volumeId'], snapshot.id))

            snapshot_tags = {'Name': "{0} - {3} [{1} - {2}]".format(
                self.depl.description, self.name, k, backup_id)}
    ...
    def restore(self, defn, backup_id, devices=[]):
        ...
        self.log("restoring machine ‘{0}’ to backup "
                 "‘{1}’".format(self.name, backup_id))
        for d in devices:
            self.log(" - {0}".format(d))
        ...
        for k, v in self.block_device_mapping.items():
            if devices == [] or _sd_to_xvd(k) in devices:
                ...
                self.log("creating volume from snapshot "
                         "‘{0}’".format(snapshot_id))
                new_volume = self._conn.create_volume(
                    size=0, snapshot=snapshot_id, zone=self.zone)

                # check if original volume is available, aka detached from the
                # machine
                self.wait_for_volume_available(volume)
                # check if new volume is available
                self.wait_for_volume_available(new_volume)

                self.log("attaching volume ‘{0}’ to "
                         "‘{1}’".format(new_volume.id, self.name))
                new_volume.attach(self.vm_id, k)
                new_v = self.block_device_mapping[k]
                if v.get('partOfImage', False) or\
                        v.get('charonDeleteOnTermination', False) or\
                        v.get('deleteOnTermination', False):
    ...
    def attach_volume(self, device, volume_id):
        ...
        if volume.status == "in-use" and \
            ...
            self.log_start("detaching volume ‘{0}’ from instance ‘{1}’"
                           "...".format(volume_id,
                                        volume.attach_data.instance_id))
            volume.detach()
            ...
            if volume.update() != "available":
                self.log("force detaching volume ‘{0}’ from instance ‘{1}’"
                         "...".format(volume_id,
                                      volume.attach_data.instance_id))
                volume.detach(True)
                nixops.util.check_wait(check_available)

        self.log_start("attaching volume ‘{0}’ as ‘{1}’"
                       "...".format(volume_id, _sd_to_xvd(device)))
        ...
        def check_dev():
            res = self.run_command("test -e {0}".format(
                _sd_to_xvd(device)), check=False)
            return res == 0
    ...
    def assign_elastic_ip(self, elastic_ipv4, instance, check):
        ...
        if (self.elastic_ipv4 or "") != elastic_ipv4 or\
                (instance.ip_address != elastic_ipv4) or check:
            if elastic_ipv4 != "":
                ...
                while True:
                    ...
                    if instance.state not in {"running", "pending"}:
                        raise Exception(
                            "EC2 instance ‘{0}’ failed to reach running "
                            "state (state is ‘{1}’)".format(
                ...
                addresses = self._conn.get_all_addresses(
                    addresses=[elastic_ipv4])
                if addresses[0].instance_id != "" \
                    and addresses[0].instance_id != self.vm_id \
                    and not self.depl.confirm(
                        "are you sure you want to associate IP address ‘{0}’, "
                        "which is currently in use by instance ‘{1}’?".format(
                            elastic_ipv4, addresses[0].instance_id)):
                    raise Exception("elastic IP ‘{0}’ already in use"
                                    "...".format(elastic_ipv4))
                else:
                    self.log("associating IP address ‘{0}’"
                             "...".format(elastic_ipv4))
                    addresses[0].associate(self.vm_id)
                    self.log_start("waiting for address to be associated with "
                                   "this machine...")
                    instance.update()
                    while True:
                ...
                self.log("disassociating IP address ‘{0}’"
                         "...".format(self.elastic_ipv4))
    ...
    def create(self, defn, check, allow_reboot, allow_recreate):
        ...
        # such as the type.
        if self.vm_id and allow_reboot and self._booted_from_ebs() and\
        ...
        if self.vm_id and check:
            ...
            if instance is None or\
                    instance.state in {"shutting-down", "terminated"}:
                if not allow_recreate:
                    raise Exception("EC2 instance ‘{0}’ went away; use "
                                    "‘--allow-recreate’ to create a new "
                                    "one".format(self.name))
                self.log("EC2 instance went away (state ‘{0}’), will "
                         "recreate".format(
                ...
                if self.instance_type != defn.instance_type:
                    self.log("changing instance type from ‘{0}’ to ‘{1}’"
                ...
                # When we restart, we'll probably get a new IP.  So forget the
                # current one.
                self.public_ipv4 = None
                self.private_ipv4 = None
        ...
        # Create the instance.
        if not self.vm_id:
            self.log("creating EC2 instance (AMI ‘{0}’, type ‘{1}’, region "
                     "‘{2}’)...".format(defn.ami, defn.instance_type,
                                            defn.region))
            ...
            resize_root = defn.root_disk_size != 0 and\
            ...
            for k, v in defn.block_device_mapping.iteritems():
                if re.match("/dev/sd[a-e]", k) and not\
                    ...
                    raise Exception("non-ephemeral disk not allowed on "
                                    "device ‘{0}’; use /dev/xvdf or "
                                    "higher".format(_sd_to_xvd(k)))
                if v['disk'].startswith("ephemeral"):
                    devmap[k] = boto.ec2.blockdevicemapping.BlockDeviceType(
                        ephemeral_name=v['disk'])
            ...
            for k, v in defn.block_device_mapping.iteritems():
                ...
                if not zone:
                    self.log("starting EC2 instance in zone ‘{0}’ due to "
                             "volume ‘{1}’".format(volume.zone, v['disk']))
                    zone = volume.zone
                elif zone != volume.zone:
                    raise Exception("unable to start EC2 instance ‘{0}’ in "
                                    "zone ‘{1}’ because volume ‘{2}’ is in "
                                    "zone ‘{3}’".format(
                                        self.name, zone, v['disk'],
                                        volume.zone))
            ...
            if not self.client_token:
                with self.depl._db:
                    self.client_token = nixops.util.generate_random_string(
                        length=48)  # = 64 ASCII chars
                    self.state = self.STARTING

            reservation = nixops.ec2_utils.retry(
                lambda: self._conn.run_instances(
                ...
                error_codes=['InvalidParameterValue', 'UnauthorizedOperation'])
        ...
        while True:
            try:
                ...
                if e.error_code != "InvalidInstanceID.NotFound":
                    raise
            self.log("EC2 instance ‘{0}’ not known yet, waiting"
                     "...".format(self.vm_id))
            time.sleep(3)

        # Warn about some EC2 options that we cannot update for an existing
        # instance.
        if self.instance_type != defn.instance_type:
            self.warn("cannot change type of a running instance (use "
                      "‘--allow-reboot’)")
        ...
        for k, v in self.block_device_mapping.items():
            if k not in defn.block_device_mapping and not\
                    v.get('partOfImage', False):
                if v['disk'].startswith("ephemeral"):
                    raise Exception("cannot detach ephemeral device ‘{0}’ "
                                    "from EC2 instance ‘{1}’"
                ...
                volumes = self._conn.get_all_volumes([], filters={
                ...
                if len(volumes) == 1:
                    ...
                    if v.get('encrypt', False):
                        ...
                        self.run_command("umount -l {0}".format(dm),
                                         check=False)
                        self.run_command("cryptsetup luksClose "
                                         "{0}".format(
                                             device.replace("/dev/", "")),
                                         check=False)
                    else:
                        self.run_command("umount -l {0}".format(device),
                                         check=False)
                    if not self._conn.detach_volume(
                            volumes[0].id, instance_id=self.vm_id, device=k):
                        raise Exception("unable to detach device ‘{0}’ from "
                                        "EC2 machine ‘{1}’".format(
                                            v['disk'], self.name))
                    # FIXME: Wait until the volume is actually detached.

                if v.get('charonDeleteOnTermination', False) or \
                        v.get('deleteOnTermination', False):
                    self._delete_volume(v['volumeId'])
        ...
        # them.
        for k, v in self.block_device_mapping.items():
            if k not in instance.block_device_mapping.keys() and not\
                    v.get('needsAttach', False) and v.get('volumeId', None):
                self.warn("device ‘{0}’ was manually detached"
                          "!".format(_sd_to_xvd(k)))
                v['needsAttach'] = True
                self.update_block_device_mapping(k, v)
        ...
        for k, v in self.block_device_mapping.items():
            if v.get('needsAttach', False):
                ...
                if not allow_recreate:
                    raise Exception(
                        "volume ‘{0}’ (used by EC2 instance ‘{1}’) no longer "
                        "exists; run ‘nixops stop’, then ‘nixops deploy "
                        "--allow-recreate’ to create a new, empty volume"
                        .format(v['volumeId'], self.name))
                self.warn("volume ‘{0}’ has disappeared; will create an empty "
                          "volume to replace it".format(v['volumeId']))
                self.update_block_device_mapping(k, None)
        ...
        for k, v in defn.block_device_mapping.iteritems():
            ...
            if v['disk'] == '':
                ...
                self.log("creating volume from snapshot ‘{0}’"
                         "...".format(v['disk']))
                (volume_type, iops) = self.disk_volume_options(v)
                volume = self._conn.create_volume(
                    size=0, snapshot=v['disk'], zone=self.zone,
                    volume_type=volume_type, iops=iops)
                v['volumeId'] = volume.id

            else:
                raise Exception("adding device mapping ‘{0}’ to a running "
            ...
            # wait for volume to get to available state for newly created
            # volumes only (amazon sometimes returns weird temporary states for
            # newly created volumes, e.g. shortly in-use). doing this after
            # updating the device mapping state, to make it recoverable in case
            # an exception happens (e.g. in other machine's deployments).
            if volume:
                self.wait_for_volume_available(volume)
        ...
        for k, v in self.block_device_mapping.items():
            ...
            volume_tags['Name'] = "{0} [{1} - {2}]".format(
                self.depl.description, self.name, _sd_to_xvd(k))
        ...
        for k, v in self.block_device_mapping.items():
            if v.get('encrypt', False) and v.get('passphrase', "") == "" and\
                    v.get('generatedKey', "") == "":
                v['generatedKey'] = nixops.util.generate_random_string(
                    length=256)
                self.update_block_device_mapping(k, v)
    ...
    def _update_route53(self, defn):
        ...
        self.log('sending Route53 DNS: {0} {1}'.format(
            self.public_ipv4, self.dns_hostname))
        ...
        zones = [zone for zone in zones[
            ...
            if "{0}.".format(hosted_zone) == zone.Name]
        if len(zones) != 1:
            raise Exception('hosted zone for {0} not '
                            'found'.format(hosted_zone))
        zoneid = zones[0]['Id'].split("/")[2]

        # name argument does not filter, just is a starting point, annoying..
        # copying into a separate list
        all_prevrrs = self._conn_route53.get_all_rrsets(
            hosted_zone_id=zoneid, type="A", name="{0}.".format(
                self.dns_hostname))
        prevrrs = []
    ...
    def _delete_volume(self, volume_id):
        if not self.depl.confirm("are you sure you want to destroy EC2 volume "
                                 "‘{0}’?".format(volume_id)):
            raise Exception("not destroying EC2 volume "
                            "‘{0}’".format(volume_id))
    ...
    def destroy(self):
        if not (self.vm_id or self.client_token):
            return True
        if not self.depl.confirm("are you sure you want to destroy EC2 "
                                 "machine ‘{0}’?".format(self.name)):
            return False
    ...
    def stop(self):
        ...
        if not nixops.util.check_wait(check_stopped, initial=3, max_tries=300,
                                      exception=False):  # = 15 min
            # If stopping times out, then do an unclean shutdown.
            self.log_end("(timed out)")
            self.log_start("force-stopping EC2 machine... ")
            instance.stop(force=True)
            if not nixops.util.check_wait(check_stopped, initial=3,
                                          max_tries=100,
                                          exception=False):  # = 5 min
                # Amazon docs suggest doing a force stop twice...
                self.log_end("(timed out)")
                self.log_start("force-stopping EC2 machine... ")
                instance.stop(force=True)
                nixops.util.check_wait(check_stopped, initial=3,
                                       max_tries=100)  # = 5 min
    ...
    def start(self):
        ...
        if prev_private_ipv4 != self.private_ipv4 or\
                prev_public_ipv4 != self.public_ipv4:
    ...
    def _check(self, res):
        ...
        #self.log("instance state is ‘{0}’".format(
        #    instance.state if instance else "gone"))
        ...
        if instance.state == "pending":
            ...
            res.disks_ok = True
            for k, v in self.block_device_mapping.items():
                if k not in instance.block_device_mapping.keys() and\
                        v.get('volumeId', None):
                    res.disks_ok = False
                    res.messages.append("volume ‘{0}’ not attached to "
                                        "‘{1}’".format(v['volumeId'],
                                                           _sd_to_xvd(k)))
                    volume = self._get_volume_by_id(v['volumeId'],
                                                    allow_missing=True)
                    if not volume:
                        res.messages.append("volume ‘{0}’ no longer "
                                            "exists".format(v['volumeId']))

                if k in instance.block_device_mapping.keys() and\
                        instance.block_device_mapping[k].status != 'attached':
                    res.disks_ok = False
                    res.messages.append(
                        "volume ‘{0}’ on device ‘{1}’ has unexpected state: "
                        "‘{2}’".format(
                            v['volumeId'], _sd_to_xvd(k),
                            instance.block_device_mapping[k].status))

            if self.private_ipv4 != instance.private_ip_address or\
                    self.public_ipv4 != instance.ip_address:
                self.warn("IP address has changed, you may need to run "
                          "‘nixops deploy’")
                self.private_ipv4 = instance.private_ip_address
        ...
        instance_status = self._conn.get_all_instance_status(
        ...
        for ist in instance_status:
            if ist.events:
                for e in ist.events:
                    ...
                    res.messages.append("  * {0} - {1}".format(
    ...
    def get_console_output(self):
        if not self.vm_id:
            raise Exception("cannot get console output of non-existant "
                            "machine ‘{0}’".format(self.name))
        self.connect()
        return self._conn.get_console_output(self.vm_id).output or\
            "(not available)"
