...
def create_definition(xml):
    ...
    for i in [charon.backends.none.NoneDefinition, charon.backends.virtualbox.VirtualBoxDefinition]:
...
def create_state(depl, type, name):
    ...
    for i in [charon.backends.none.NoneState, charon.backends.virtualbox.VirtualBoxState]:
        if type == i.get_type():
            return i(depl, name)
